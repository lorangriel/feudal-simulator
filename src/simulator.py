...
def load_worlds_from_file():
    ...
    if os.path.exists(DEFAULT_WORLDS_FILE):
        try:
            ...
            messagebox.showerror("Laddningsfel", f"Kunde inte läsa filen {DEFAULT_WORLDS_FILE}.\nFilen kan vara korrupt.\n\n{e}")
            return {}
        except Exception as e:
            print(f"Error loading file {DEFAULT_WORLDS_FILE}: {e}")
            messagebox.showerror("Laddningsfel", f"Ett oväntat fel uppstod vid läsning av {DEFAULT_WORLDS_FILE}.\n\n{e}")
            return {}
    else:
        return {}
...
def save_worlds_to_file(all_worlds):
    ...
    try:
        ...
        messagebox.showerror("Sparfel", f"Kunde inte spara data till {DEFAULT_WORLDS_FILE}.\n\n{e}")
...
def roll_dice(expr: str, debug=False):
    ...
    try:
        ...
        if die_type <= 0: raise ValueError("Die type must be positive")
    ...
    if not unlimited:
        ...
        if debug:
            ...
            return total, ""
    else: # Unlimited/Exploding dice
        queue = dice_count
        roll_count = 0
        while queue > 0 and roll_count < 100: # Safety break for extreme rolls
            val = random.randint(1, die_type)
            queue -= 1
            roll_count += 1
            if val == die_type:
                details.append(f"{die_type}->+2 nya")
                queue += 2 # Add two more dice to roll
            else:
        ...
        if debug:
            ...
            if roll_count >= 100: dbg_rolls += " (MAX ROLLS)"
...
# --------------------------------------------------
# Dynamic Map (Not currently used for hex grid)
# NOTE: This class remains largely unchanged from ...52.txt
# --------------------------------------------------
class DynamicMapCanvas:
    """
    A simplified dynamic map using random placement and lines.
    NOTE: This is separate from the static hex map view.
    """
    def __init__(self, parent_frame, simulator, world_data):
        ...
        self.positions = {} # Store node_id -> (x, y)
    ...
    def show(self):
        ...
        self.canvas.bind("<MouseWheel>", self.on_dynamic_map_zoom) # Windows/macOS
        self.canvas.bind("<Button-4>", self.on_dynamic_map_zoom) # Linux scroll up
        self.canvas.bind("<Button-5>", self.on_dynamic_map_zoom) # Linux scroll down
    ...
    def on_dynamic_map_zoom(self, event):
        ...
        # Rescale all items on canvas
        # Use canvas center as zoom origin for better feel
    ...
    def draw_dynamic_map(self):
        ...
        if self.world_data and "nodes" in self.world_data:
            for node_id_str, nd in self.world_data["nodes"].items():
                # Ensure node_id is int for depth check if needed
                try:
                    node_id = int(node_id_str)
                    if self.simulator.get_depth_of_node(node_id) == 3:
                        jarldomes.append(nd)
                except ValueError:
                    continue # Skip if node_id is not an integer string
        ...
        # Draw nodes (polygons and text)
        node_polygons = {} # Store polygon info for line drawing
        for nd in jarldomes:
            jid = nd["node_id"]
            x, y = self.positions[jid]

            # Determine number of sides based on actual neighbors (optional, could be fixed)
            neighbor_count = 0
            if "neighbors" in nd:
                for nb in nd["neighbors"]:
                    nb_id = nb.get("id")
                    # Check if it's a valid Jarldom ID (int)
                    if isinstance(nb_id, int) and str(nb_id) in self.world_data.get("nodes", {}):
                        neighbor_count += 1

            sides = max(3, min(neighbor_count, 8)) # Use 3-8 sides
            ...
            color_fill = "#ffdddd" # Default fill
            outline_color = "red"  # Default outline

            poly_id = self.canvas.create_polygon(pts, fill=color_fill, outline=outline_color, width=2)
            # Use the same display name logic as the tree view
            ...
            # Bind click event to show node view (Double click)
    ...
    def draw_dynamic_lines(self, node_polygons):
        ...
        drawn_pairs = set() # To avoid drawing lines twice (A->B and B->A)
        ...
        for node_id_str, nd in self.world_data["nodes"].items():
            ...
            if self.simulator.get_depth_of_node(node_id) == 3 and "neighbors" in nd:
                ...
                if A_id not in node_polygons:
                    continue # Skip if this node wasn't drawn (shouldn't happen)
                ...
                for nb_info in nd.get("neighbors", []):
                    ...
                    # Check if neighbor is a valid Jarldom ID and exists on map
...
class FeodalSimulator:
    ...
    def load_world(self, wname):
        ...
        if "characters" not in self.world_data: self.world_data["characters"] = {}
        if "next_node_id" not in self.world_data:
            # Calculate next_node_id if missing
            max_id = 0
            ...
            print(f"Initialized missing next_node_id to {self.world_data['next_node_id']}")

        # Ensure basic fields exist in nodes and characters
        nodes_updated = 0
        chars_updated = 0
        max_node_id_found = 0
        all_node_ids = set()
        ...
        for nid_str, node in self.world_data.get("nodes", {}).items():
            try:
                ...
                updated = False
                if "node_id" not in node: node["node_id"] = nid_int; updated = True
                if "parent_id" not in node: node["parent_id"] = None; # Or guess? Risky. Assume None if missing.
                if "name" not in node: node["name"] = ""; updated = True
                if "custom_name" not in node: node["custom_name"] = ""; updated = True
                if "ruler_id" not in node: node["ruler_id"] = None
                if "num_subfiefs" not in node: node["num_subfiefs"] = 0; updated = True
                if "children" not in node: node["children"] = []; updated = True
                if "population" not in node: node["population"] = 0; updated = True
                # Ensure children are ints if possible
                node["children"] = [int(c) for c in node.get("children", []) if str(c).isdigit()]

                depth = self.get_depth_of_node(nid_int) # Need to calculate depth first
                if depth == 3: # Jarldom specific checks
                    if "neighbors" not in node:
                        node["neighbors"] = [{"id": None, "border": NEIGHBOR_NONE_STR} for _ in range(MAX_NEIGHBORS)]
                        updated = True
                    else: # Ensure list is correct length and format
                        neighbors = node["neighbors"]
                        if not isinstance(neighbors, list): neighbors = []
                        validated_neighbors = []
                        for i in range(MAX_NEIGHBORS):
                            if i < len(neighbors) and isinstance(neighbors[i], dict):
                                ...
                                # Convert ID to int if possible, handle None and OTHER_STR
                                final_id = None
                                if isinstance(n_id, int): final_id = n_id
                                elif str(n_id).isdigit(): final_id = int(n_id)
                                elif n_id == NEIGHBOR_OTHER_STR: final_id = NEIGHBOR_OTHER_STR
                        ...
                        if node["neighbors"] != validated_neighbors:
                            node["neighbors"] = validated_neighbors
                            updated = True
                elif depth >= 4: # Resource specific checks
                    if "res_type" not in node: node["res_type"] = "Resurs"; updated = True
                    # Add checks for count, area_size etc based on type if needed

                if updated: nodes_updated += 1
            except ValueError:
                print(f"Skipping node with non-integer key: {nid_str}")
                del self.world_data["nodes"][nid_str] # Remove invalid node entry

        # Validate parent_id and children links
        nodes_to_remove = set()
        for nid_str, node in self.world_data.get("nodes", {}).items():
            nid_int = int(nid_str)
            # Check parent exists
            ...
            if parent_id is not None and parent_id not in all_node_ids:
                print(f"Warning: Node {nid_int} has invalid parent_id {parent_id}. Setting parent to None.")
                node["parent_id"] = None
                nodes_updated += 1
            # Check children exist and fix parent link if needed
            valid_children = []
            for child_id in node.get("children", []):
                if child_id in all_node_ids:
                    child_node = self.world_data["nodes"].get(str(child_id))
                    if child_node:
                        # Ensure child points back correctly
                        if child_node.get("parent_id") != nid_int:
                            print(f"Warning: Child {child_id} parent_id ({child_node.get('parent_id')}) doesn't match parent {nid_int}. Fixing child.")
                            child_node["parent_id"] = nid_int
                            nodes_updated += 1
                        valid_children.append(child_id)
                    else: # Should not happen if child_id in all_node_ids
                        print(f"Error: Child node {child_id} found in ID set but not in dictionary.")
                else:
                    print(f"Warning: Node {nid_int} has invalid child_id {child_id}. Removing child link.")
                    nodes_updated += 1
        ...
        for cid_str, char in self.world_data.get("characters", {}).items():
            updated = False
            if "char_id" not in char: char["char_id"] = int(cid_str); updated = True # Add internal ID if missing
            if "name" not in char: char["name"] = ""; updated = True
            if "wealth" not in char: char["wealth"] = 0; updated = True
            if "description" not in char: char["description"] = ""; updated = True
            if "skills" not in char: char["skills"] = []; updated = True
            if updated: chars_updated += 1

        # Validate ruler_id links in nodes
        ...
        for node in self.world_data.get("nodes", {}).values():
            ...
            if ruler_id is not None and str(ruler_id) not in valid_char_ids:
                print(f"Warning: Node {node.get('node_id','??')} has invalid ruler_id {ruler_id}. Clearing ruler.")
                node["ruler_id"] = None
                nodes_updated += 1
        ...
        if self.world_data.get("next_node_id", 1) < calculated_next_id:
            print(f"Warning: World next_node_id ({self.world_data.get('next_node_id', 1)}) was lower than max found ID ({max_node_id_found}). Updating.")
            self.world_data["next_node_id"] = calculated_next_id
            nodes_updated += 1 # Count this as an update

        if nodes_updated > 0 or chars_updated > 0:
            self.add_status_message(f"Validerade och uppdaterade data vid laddning: {nodes_updated} noder, {chars_updated} härskare.")
            self.save_current_world() # Save fixes immediately
